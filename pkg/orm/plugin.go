...
const (
	createStartTimeKey = "gorm:create_start_time"
	queryStartTimeKey  = "gorm:query_start_time"
)

var _ gorm.Plugin = (*CustomePlugin)(nil)
var _ gorm.Plugin = &CustomePlugin{}
...
func (p *CustomePlugin) Initialize(db *gorm.DB) error {
	...
	if err := db.Callback().Create().Before("gorm:createBefore").Register("gorm:createBefore:metric:trace", func(db *gorm.DB) {
		...
		// db.InstanceSet(createStartTimeKey+string(i), startTime)
		db.InstanceSet(createStartTimeKey, startTime)
	...
	if err := db.Callback().Create().After("gorm:createAfter").Register("gorm:createAfter:metric:trace", func(db *gorm.DB) {
		startTime, ok := db.InstanceGet(createStartTimeKey)
		if !ok {
			return
		}
